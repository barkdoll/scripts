...
def format_aozora_fnames(fname):
    # Special file name format processing
    # name format: TITLE // AUTHOR（著）
    substitutions = [
        {
            'pattern': r'(?P<author>.+) –– (?P<title>.+)',
            'replacement': r'\g<title> // \g<author>（著）'
        },
        {
            'pattern': r"([^ ])(\(校正)",
    ...
    for s in substitutions:
        fname = re.sub(s['pattern'], s['replacement'], fname)
...
if __name__ == '__main__':
    ...
    headline = create_headline(targets)
    ...
    for file in tqdm(DATA_DIR.glob('**/*')):
        ...
        with open(file, 'rb') as current_file:
            ...
            if (re.search(rex, text)):
                ...
                # the first (zero) index of each tuple is always
                # the full sentence.
                ...
                file_name = (
                    str(file) if not aozora_mode
                    else format_aozora_fnames(str(file)))
    ...
    print(
        '\n' +
        f'Files found:\t\t', colored(file_counter, 'cyan'))
